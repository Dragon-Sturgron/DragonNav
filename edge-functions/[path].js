...
<div id="appView" class="wrap hidden">
  ...
  <div id="sitePage" class="hidden">
    ...
    <section class="card">
      ...
      <div id="siteTable" class="tableWrap"><table><thead><tr><th>拖拽</th><th>网站</th><th>地址</th><th>启用</th><th>操作</th></tr></thead><tbody id="siteBody"></tbody></table></div>
...
<div id="siteModal" class="modalBack"><div class="modal"><h3 id="siteModalTitle">编辑网站</h3><div class="row"><div class="field"><label>网站名称</label><input id="siteName"></div><div class="field"><label>分类</label><select id="siteCategory"></select></div><div class="field full"><label>网站地址</label><input id="siteUrl" placeholder="https://example.com/"></div><div class="field full"><label>图标 URL（可留空）</label><input id="siteIcon"></div><div class="field full"><label>说明</label><input id="siteDesc"></div><div class="field"><label>状态</label><select id="siteEnabled"><option value="true">启用</option><option value="false">停用</option></select></div></div><div class="modalFoot"><button data-close="siteModal" class="btn light">取消</button><button id="saveSiteBtn" class="btn primary">保存</button></div></div></div>
...
function openSiteModal(id){editingSite=id||'';fillCategories();var s=id?config.sites.find(function(v){return v.id===id}):null;$('siteModalTitle').textContent=s?'编辑网站':'新增网站';$('siteName').value=s?s.name:'';$('siteUrl').value=s?s.url:'';$('siteIcon').value=s?s.icon||'':'';$('siteDesc').value=s?s.desc||'':'';$('siteCategory').value=s?s.categoryId:selectedCategory;$('siteEnabled').value=String(s?s.enabled!==false:true);modal('siteModal',true)}
function saveSite(){var name=$('siteName').value.trim(),url=$('siteUrl').value.trim(),categoryId=$('siteCategory').value;if(!name||!url||!categoryId)return alert('名称、地址和分类不能为空。');try{var u=new URL(url);if(['http:','https:'].indexOf(u.protocol)<0)throw 0}catch(e){return alert('请输入有效的 http/https 网站地址。')}var data={name:name,url:url,icon:$('siteIcon').value.trim(),desc:$('siteDesc').value.trim(),categoryId:categoryId,enabled:$('siteEnabled').value==='true'};if(editingSite){var old=config.sites.find(function(v){return v.id===editingSite});var moved=old&&old.categoryId!==categoryId;if(old)Object.assign(old,data);if(moved){config.sites=config.sites.filter(function(v){return v.id!==editingSite});config.sites.push(old)}}else config.sites.push(Object.assign({id:uid('site')},data));modal('siteModal',false);renderSites();markDirty()}
